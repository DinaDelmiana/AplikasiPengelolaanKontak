...
public class KontakDAO {
    ...
    public List<Kontak> searchContacts(String keyword) throws SQLException {
        ...
        try (Connection conn = DatabaseConnection.getConnection(); PreparedStatement pstmt = conn.prepareStatement(sql)) {
            ...
            while (rs.next()) {
                Kontak contact = new Kontak(rs.getInt("id"), rs.getString("nama"), rs.getString("nomor_telepon"), rs.getString("kategori"));
                contacts.add(contact);
            }
        }
        return contacts;
    ...
    public boolean isDuplicatePhoneNumber(String nomorTelepon, Integer excludeId) throws SQLException {
        String sql = "SELECT COUNT(*) FROM contacts WHERE nomor_telepon = ?" + (excludeId != null ? " AND id != ?" : "");
        try (Connection conn = DatabaseConnection.getConnection(); PreparedStatement pstmt = conn.prepareStatement(sql)) {
